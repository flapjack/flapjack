...
"""
  ...
  Scenario: Starting flapper
    When I start flapper (via bundle exec) with `flapjack --config tmp/cucumber_cli/flapjack_cfg.toml flapper start --no-daemonize`
  ...
  Scenario: Starting, status, and stopping flapper, daemonized
    ...
    Then flapper should start within 15 seconds
    When I run `bundle exec bin/flapjack --config tmp/cucumber_cli/flapjack_cfg.toml flapper status -p tmp/cucumber_cli/flapper.pid`
    Then the exit status should be 0
    And  the output should contain "flapper is running"
    When I stop flapper (via bundle exec) with `flapjack --config tmp/cucumber_cli/flapjack_cfg.toml flapper stop -p tmp/cucumber_cli/flapper.pid`
    Then flapper should stop within 15 seconds
