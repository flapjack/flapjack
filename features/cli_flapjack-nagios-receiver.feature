...
"""
  ...
  Scenario: Starting nagios-receiver
    When I start nagios-receiver (via bundle exec) with `flapjack -n test --config tmp/cucumber_cli/nagios-receiver.toml receiver start --no-daemonize`
    Then nagios-receiver should start within 15 seconds

  Scenario: Stopping nagios-receiver via SIGINT
    When I start nagios-receiver (via bundle exec) with `flapjack -n test --config tmp/cucumber_cli/nagios-receiver.toml receiver nagios start --no-daemonize`
    Then nagios-receiver should start within 15 seconds
  ...
  Scenario: Starting, status, and stopping nagios-receiver, daemonized
    When I start nagios-receiver (daemonised) (via bundle exec) with `flapjack -n test --config tmp/cucumber_cli/nagios-receiver.toml receiver nagios start -d`
    Then nagios-receiver should start within 15 seconds
    When I run `bundle exec bin/flapjack -n test --config tmp/cucumber_cli/nagios-receiver.toml receiver nagios status`
    Then the exit status should be 0
    And  the output should contain "nagios-receiver is running"
    When I stop nagios-receiver (via bundle exec) with `flapjack -n test --config tmp/cucumber_cli/nagios-receiver.toml receiver nagios stop`
    Then nagios-receiver should stop within 15 seconds

  Scenario: Starting, restarting and stopping nagios-receiver, daemonized
    When I start nagios-receiver (daemonised) (via bundle exec) with `flapjack  -n test --config tmp/cucumber_cli/nagios-receiver.toml receiver nagios start -d`
    Then nagios-receiver should start within 15 seconds
    When I restart nagios-receiver (daemonised) (via bundle exec) with `flapjack -n test --config tmp/cucumber_cli/nagios-receiver.toml receiver nagios restart`
