...
Given /^user (\S+) has the following notification rules:$/ do |contact_id, rules|
  ...
  rules.hashes.each do |rule|
    ...
    rule_data = {:time_restrictions  => time_restrictions, :is_specific => false}
    ...
    tags.each do |tag_name|
      ...
      new_rule.tags << tag
      unless new_rule.is_specific
        new_rule.is_specific = true
        new_rule.save
      end
    end
    ...
    contact.notification_rules << new_rule
  end
end
