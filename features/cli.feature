...
"""
  ...
  Scenario: Running with --help shows usage information
    When I run `bin/flapjack-main --help`
    Then the exit status should be 0
    And  the output should contain "Usage: flapjack-main"
    And  the output should contain " reload "
    And  the output should contain "-c, --config"

  Scenario: Getting status when stopped
    When I run `bin/flapjack-main status --config tmp/cucumber_cli/flapjack_cfg_d.yaml`
    Then the exit status should not be 0
    And  the output should contain "Flapjack is not running"

  Scenario: Starting flapjack
    When I start flapjack with `flapjack-main start --config tmp/cucumber_cli/flapjack_cfg.yaml`
  ...
  Scenario: Starting, status and stopping flapjack, daemonized
    When I start flapjack (daemonised) with `flapjack-main start -d --config tmp/cucumber_cli/flapjack_cfg_d.yaml`
    Then flapjack should start within 15 seconds
    When I run `bin/flapjack-main status --config tmp/cucumber_cli/flapjack_cfg_d.yaml`
    Then the exit status should be 0
    And  the output should contain "Flapjack is running"
    When I stop flapjack with `flapjack-main stop --config tmp/cucumber_cli/flapjack_cfg_d.yaml`
    Then flapjack should stop within 15 seconds

  Scenario: Starting, restarting and stopping flapjack, daemonized
    When I start flapjack (daemonised) with `flapjack-main start -d --config tmp/cucumber_cli/flapjack_cfg_d.yaml`
    Then flapjack should start within 15 seconds
    When I restart flapjack with `flapjack-main restart -d --config tmp/cucumber_cli/flapjack_cfg_d.yaml`
