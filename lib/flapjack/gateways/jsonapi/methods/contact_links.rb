        ...
        module ContactLinks
          ...
          def self.registered(app)
            ...
            app.post %r{^/contacts/(#{Flapjack::UUID_RE})/links/(media|rules)$} do
              ...
              resource_post_links(Flapjack::Data::Contact, 'contacts', contact_id,
                                  assoc_type)
              status 204
            end
            ...
            app.get %r{^/contacts/(#{Flapjack::UUID_RE})/(media|rules)} do
              ...
              status 200
              resource_get_links(Flapjack::Data::Contact, 'contacts', contact_id,
                                 assoc_type)
            end

            app.patch %r{^/contacts/(#{Flapjack::UUID_RE})/links/(media|rules)$} do
              contact_id = params[:captures][0]
              assoc_type = params[:captures][1]

              resource_patch_links(Flapjack::Data::Contact, 'contacts', contact_id,
                                   assoc_type)
              status 204
            end
