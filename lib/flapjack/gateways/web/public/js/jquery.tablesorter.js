    ...
    format: function(s) {
      ...
      for (var i=0; i<items.length; i++) {
        if (items[i].indexOf("y") != -1) {
          years = items[i].replace('y', '');
        } else if (items[i].indexOf("mo") != -1) {
          months = items[i].replace('mo', '');
        } else if (items[i].indexOf("d") != -1) {
          days = items[i].replace('d', '').replace('-', '');
        } else if (items[i].indexOf("h") != -1) {
          hours = items[i].replace('h', '');
        } else if (items[i].indexOf("m") != -1) {
          minutes = items[i].replace('m', '');
        } else if (items[i].indexOf("s") != -1) {
          seconds = items[i].replace('s', '');
        }
      }
      return moment().subtract('years', years).subtract('months', months).subtract('days', days).subtract('hours', hours).subtract('minutes', minutes).subtract('seconds', seconds).unix()*-1;
    },
    type: "numeric"
  });
