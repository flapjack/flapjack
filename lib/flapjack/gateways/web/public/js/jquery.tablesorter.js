...
!(function($) {
  ...
  ts.addParser({
    ...
    format: function(s) {
      if (!(/^(\d{1,2})(\S?)\s(\d{1,2})(\S?).*$/g).test(s)) {
        return 9999999999999999999;
      };

      var r = /^(\d{1,2})(\S?)\s(\d{1,2})(\S?).*$/g;
      var return_val = 0;
      var m = r.exec(s);
      var h = {};
      h[m[2]] = parseInt(m[1]);
      ...
      var minutes2sec = function(i) {
        if (iszero(i)) {
          return 0;
        }
        return i*60;
      }
      var hours2sec = function(i) {
        if (iszero(i)) {
          return 0;
        }
        return i*3600;
      }
      var days2sec = function(i) {
        if (iszero(i)) {
          return 0;
        }
        return i*86400;
      ...
      Object.keys(h).forEach(function (k) { 
        if (k == 's') {
          ...
          return_val += minutes2sec(h[k]);
        } else if (k == 'h') {
          return_val += hours2sec(h[k]);
        } else if (k == 'd') {
          return_val += days2sec(h[k]);
        } 
      })
