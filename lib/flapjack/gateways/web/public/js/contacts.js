...
var app = {}; // namespace
  ...
  app.Contact = Backbone.Model.extend({
    ...
    // handle the different attribute cases
    constructor: function() {
      var attrs = arguments[0];
      var translated = {};

      _.each(this.attrMap, function(value, key, list) {
        translated[key] = attrs[value];
      });

      Backbone.Model.apply(this, [translated]);
    }
  ...
  app.ContactCollection = Backbone.Collection.extend({
    ...
    // TODO use current API host/port when served by Sinatra
  ...
  app.ContactListItem = Backbone.View.extend({
    ...
    save: function() {
      this.model.save({
        firstName: this.$el.find('input.contact_first_name').val(),
        lastName:  this.$el.find('input.contact_last_name').val(),
