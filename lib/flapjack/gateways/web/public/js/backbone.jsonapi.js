...
toolbox.resolveLinks = function(context, obj, linked) {
  ...
  _.each(obj.links, function(ids, name) {
    ...
    var linkages = context.linkages;

    if ( _.isUndefined(linkages) ) {
      obj.links[name] = null;
      return;
    }

    var linkageType = linkages[name];

    if ( _.isUndefined(linkageType) ) {
      obj.links[name] = null;
      return;
    }
    ...
    if ( _.isUndefined(linked_for_name) ) {
      if ( _.isArray(ids) ) {
        obj.links[name] = new linkageType();
      } else {
        obj.links[name] = null;
      }
      return;
    ...
    if ( _.isArray(ids) ) {
      var collection = new linkageType();
      ...
      obj.links[name] = new linkageType(toolbox.findById(linked_for_name, ids));
    }
  });
