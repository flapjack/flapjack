(function(flapjack, Contact) {
  ...
  Contact.Views.ContactDetails = Backbone.View.extend({
    ...
    setFirstName: function(event) {
      ...
      $('button#contactAccept').prop('disabled', !this.model.isValid());
    },
    ...
    accept: function(event) {
      ...
      if ( this.model.isNew() ) {
        ...
        this.model.save({}, {type: 'POST', contentType: 'application/vnd.api+json',
                          success: save_success, error: save_error});
      } else {

        if ( _.isUndefined(this.model.clean) ) {
          $('#contactModal').modal('hide');
        } else {
          var changedAttrKeys = _.keys(this.model.clean).length;
          ...
          if (changedAttrKeys.length > 0) {
            ...
            this.model.patch('contacts', {}, {success: save_success, error: save_error});
          } else {
            $('#contactModal').modal('hide');
          }
        }
  ...
  Contact.Views.Media = Backbone.View.extend({
    ...
    render: function() {
      ...
      var mediaList = new Contact.Views.MediaList({collection: this.options.collection, contact: this.options.contact});
      this.$el.find('tbody#contactMediaList').replaceWith(mediaList.render().$el);
  ...
  Contact.Views.MediaListItem = Backbone.View.extend({
    ...
    events: {
      ...
      "change input[name='address']"          : 'setAddress',
      "change input[name='interval']"         : 'setInterval',
      "change input[name='rollup_threshold']" : 'setRollupThreshold'
    },
    initialize: function() {
    ...
    setAddress: function(event) {
      ...
      $('#contactModal button#contactAccept').prop('disabled', !this.model.isValid());
    },
  ...
  Contact.Views.Entities = Backbone.View.extend({
    ...
    render: function() {
      ...
      this.$el.find('div#contactEntityChooser').empty().append(entityChooser.render().$el);

      var entityList = new Contact.Views.EntityList({contact: this.options.contact, collection: this.options.current});
      this.$el.find('tbody#contactEntityList').empty().append(entityList.render().$el);
