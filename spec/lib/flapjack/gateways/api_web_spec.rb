...
describe Flapjack::Gateways::ApiWeb, :sinatra => true, :logger => true do
  ...
  def expect_stats
    expect(Flapjack::Diner).to receive(:statistics).and_return([])
    expect(Flapjack::Diner).to receive(:metrics).and_return([])
  ...
  context 'web page behaviour' do
    ...
    it "shows the state of a check" do
      ...
      expect(ok_state).to receive(:condition).and_return('ok')
