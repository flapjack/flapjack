...
describe 'Flapjack::Gateways::JSONAPI::Methods::Checks', :sinatra => true, :logger => true, :pact_fixture => true do
    ...
    expect(last_response.body).to be_json_eql(Flapjack.dump_json(:data =>
      check_data.merge(
        ...
        :links => {:self => "http://example.org/checks/#{check.id}",
                   ...
                   :contacts => "http://example.org/checks/#{check.id}/contacts",
                   :current_scheduled_maintenances => "http://example.org/checks/#{check.id}/current_scheduled_maintenances",
                   :current_state => "http://example.org/checks/#{check.id}/current_state",
                   :current_unscheduled_maintenance => "http://example.org/checks/#{check.id}/current_unscheduled_maintenance",
      ...
      [check_data.merge(
       ...
       check_2_data.merge(
        ...
        :links => {:self => "http://example.org/checks/#{check_2.id}",
                   ...
                   :contacts => "http://example.org/checks/#{check_2.id}/contacts",
                   :current_scheduled_maintenances => "http://example.org/checks/#{check_2.id}/current_scheduled_maintenances",
                   :current_state => "http://example.org/checks/#{check_2.id}/current_state",
                   :current_unscheduled_maintenance => "http://example.org/checks/#{check_2.id}/current_unscheduled_maintenance",
  ...
  it "retrieves paginated checks" do
    expect(Flapjack::Data::Check).to receive(:lock).
      with(Flapjack::Data::Tag, Flapjack::Data::Rule, Flapjack::Data::Route,
        Flapjack::Data::ScheduledMaintenance, Flapjack::Data::UnscheduledMaintenance,
        Flapjack::Data::State, Flapjack::Data::Contact, Flapjack::Data::Medium).
      and_yield
  ...
  it "retrieves one check and all its linked tag records" do
    ...
    expect(Flapjack::Data::Check).to receive(:jsonapi_type).twice.and_return('check')
