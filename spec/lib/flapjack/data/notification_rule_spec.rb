...
describe Flapjack::Data::NotificationRule, :redis => true do
  ...
  it "checks that a notification rule exists" do
    expect(Flapjack::Data::NotificationRule.exists_with_id?(existing_rule.id)).to be_truthy
    expect(Flapjack::Data::NotificationRule.exists_with_id?('not_there')).to be_falsey
  end
  ...
  it "checks if blackhole settings for a rule match a severity level" do
    ...
    expect(rule.blackhole?('warning')).to be_truthy
    expect(rule.blackhole?('critical')).to be_falsey
  end
